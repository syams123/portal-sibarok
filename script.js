...
// --- FUNGSI PENGECEKAN AKSES KONFIRMASI (CUT-OFF TGL 25) ---
function cekAksesKonfirmasi() {
    const tgl = new Date();
    const hariIni = tgl.getDate();
    
    // Jika tanggal 25 ke atas, batasi akses
    if (hariIni >= 25) {
        return {
            bolehAkses: false,
            pesan: `⚠️ Akses konfirmasi pembayaran ditutup mulai tanggal 25 setiap bulan. Silakan hubungi Ustadzah untuk pembayaran manual.`
        };
    }
...
function renderReportCard(studentId, data) {
    ...
    // --- CEK AKSES KONFIRMASI PEMBAYARAN ---
    const akses = cekAksesKonfirmasi();
    const isLunas = data.infaqStatus === true || data.infaqStatus === "Lunas";
    
    // Tentukan tampilan tombol konfirmasi
    let tombolKonfirmasiHtml = '';
    
    if (isLunas) {
        // Jika sudah lunas, tampilkan badge lunas
        tombolKonfirmasiHtml = `
            <div class="mt-3 p-2 bg-success bg-opacity-10 rounded-3 text-center">
                <i class="fas fa-check-circle text-success me-2"></i>
                <span class="text-success fw-bold">Sudah Lunas ✓</span>
            </div>`;
    } else if (!akses.bolehAkses) {
        // Jika belum lunas DAN tanggal >= 25, sembunyikan tombol konfirmasi
        tombolKonfirmasiHtml = `
            <div class="mt-3 p-2 bg-danger bg-opacity-10 rounded-3 text-center">
                <i class="fas fa-lock text-danger me-2"></i>
                <span class="text-danger small fw-bold">${akses.pesan}</span>
                <hr class="my-2">
                <p class="small text-muted mb-0">Silakan hubungi Ustadzah untuk pembayaran manual dan ambil kuitansi.</p>
            </div>`;
    } else {
        // Jika belum lunas DAN tanggal < 25, tampilkan tombol konfirmasi normal
        tombolKonfirmasiHtml = `
            <div class="mt-4">
                <div class="mb-3">
                    <label class="form-label fw-bold"><i class="fas fa-money-bill-wave me-2 text-success"></i>Metode Pembayaran:</label>
                    <select id="selectMetodeBayar" class="form-select" onchange="updatePaymentDetails()">
                        <option value="">Pilih Metode</option>
                        <option value="Tunai Langsung">💰 Tunai Langsung</option>
                        <option value="Transfer Mandiri">🏦 Transfer Bank Mandiri</option>
                        <option value="Transfer SeaBank">🏦 Transfer Bank SeaBank</option>
                        <option value="ShopeePay">📱 ShopeePay</option>
                        <option value="DANA">📱 DANA</option>
                    </select>
                </div>
                
                <div id="rekeningInfo" class="alert alert-info p-2 small d-none">
                    <i class="fas fa-info-circle me-1"></i> 
                    <span id="bankLabel">Bank</span>: <strong id="nomorRekening">-</strong><br>
                    <span id="atasNama">a.n. -</span>
                </div>
                
                <button class="btn btn-success w-100 py-2 fw-bold shadow-sm" onclick="konfirmasiBayar()">
                    <i class="fas fa-check-circle me-2"></i>Konfirmasi Pembayaran
                </button>
                <small class="text-muted d-block text-center mt-2" style="font-size: 0.7rem;">
                    <i class="fas fa-info-circle"></i> Konfirmasi sebelum tanggal 25 setiap bulan
                </small>
            </div>`;
    }

    // 5. Bagian Tanda Tangan (dengan tombol konfirmasi di atas/di bawah sesuai kebutuhan)
    contentHtml += `
    <div id="signatureWrapper" class="mt-4">
        <div class="row text-center align-items-start g-0">
            <div class="col-4">
    ...
    // Panggil fungsi signature jika diperlukan
    if (typeof checkSignatureStatus === 'function') {
...
async function cetakPDFRapor(id, btnElement) {
    ...
    try {
        ...
        const doc = await db.collection('students').doc(id).get();
        if (!doc.exists) throw new Error("Data santri tidak ditemukan");
        const data = doc.data();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Daftar emoji populer (Kakak bisa tambah sendiri di sini)
    const emojis = [
        '😊','😇','🥰','😍','🤩','😘','☺','🥳','🤗','👏',
        '👍','🙌','⭐','🌟','✨','🏆','🏅','🎓','📚','📝',
        '💪','🤲','🕌','🌙','🌸','🌹','🍃','✅','💯','🔥',
    ...
    // Generate tombol emoji secara otomatis
    emojis.forEach(emoji => {
        ...
        btn.onclick = function(e) {
            ...
            // Masukkan emoji ke posisi kursor
            textArea.value = textArea.value.substring(0, startPos) 
                           + emoji 
                           + textArea.value.substring(endPos, textArea.value.length);
            
            // Fokus kembali dan geser kursor
            textArea.focus();
...
// =============================================================================
// 🔄 AUTO-RESET INFAQ (VERSI PERBAIKAN)
// Menggunakan periode aktif (misal: "Agustus 2026") sebagai kunci reset.
// Otomatis mereset status santri begitu memasuki periode baru, kapan pun web dibuka.
// =============================================================================
async function cekDanResetInfaqBulanan() {
    try {
        // 1. Ambil nama periode aktif saat ini (contoh: "Agustus 2026")
        const infoPeriode = hitungPeriodeInfaq(); 
        const kunciReset = `reset_${infoPeriode.periode.replace(/\s+/g, '_')}`; // Hasil: "reset_Agustus_2026"

        const configRef = db.collection('app_config').doc('infaq_reset');
        const configDoc = await configRef.get();
        const configData = configDoc.exists ? configDoc.data() : {};

        // 2. Jika periode bulan ini sudah pernah di-reset, hentikan (agar tidak reset berulang)
        if (configData.lastResetKey === kunciReset) {
            console.log(`[Auto-Reset Infaq] Periode ${infoPeriode.periode} sudah di-reset sebelumnya.`);
            return;
        }

        // 3. Jika belum di-reset, lakukan reset massal untuk menyambut periode baru
        console.log(`[Auto-Reset Infaq] Memulai reset infaq untuk periode: ${infoPeriode.periode}...`);
        ...
        if (!snapshot.empty) {
            ...
            await batch.commit();
            console.log(`[Auto-Reset Infaq] Berhasil mereset ${snapshot.size} santri ke Belum Lunas.`);
        }

        // 4. Simpan kunci agar periode ini tidak di-reset lagi
        await configRef.set({
            lastResetKey: kunciReset,
            tanggalReset: firebase.firestore.FieldValue.serverTimestamp()
        });

        // Refresh tampilan dasbor otomatis setelah reset
        if (typeof renderStudents === "function") renderStudents();
